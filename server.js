...
console.log('🕋 100% HALAL EXNESS TRADING BOT - FINAL');
...
app.post('/api/set-exness-creds', authenticate, async (req, res) => {
    try {
        ...
        users[req.user.email].lastBalance = balance;
        writeUsers(users);
...
app.post('/api/start-trading', authenticate, async (req, res) => {
    try {
        ...
        if (balance < investmentAmount) {
            return res.status(400).json({ success: false, message: `Insufficient balance. You have $${balance.toFixed(2)} USD, need $${investmentAmount}` });
        }
